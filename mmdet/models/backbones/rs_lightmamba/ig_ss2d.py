...
class IGSS2D(SS2D):
    """SS2D with the cleaned v3b-style region-priority reorder path only."""
    ...
    def __init__(self,
        ...
        self.forward_core = partial(self.forward_core_ig, **core_keywords)
    ...
    def set_fg_loss_weight(self, fg_loss_weight: float) -> None:
        self.ig_scan_module.set_fg_loss_weight(fg_loss_weight)
    ...
    def forward_core_ig(self,
        ...
        if no_einsum:
            ...
            dts, Bs, Cs = torch.split(x_dbl, [dt_rank, state_dim, state_dim],
                                      dim=2)
            if hasattr(self, 'dt_projs_weight'):
        ...
        ys = selective_scan_fn(
            ...
            backend=selective_scan_backend).view(batch_size, k_group, -1,
                                                 seq_len)
